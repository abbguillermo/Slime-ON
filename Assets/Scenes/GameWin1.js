class GameWin1 extends Phaser.Scene {
    constructor() {
        super("gamewin1");
    }
    ...
    create(){
        ...
        button_next.setInteractive()
        button_next.on("pointerdown", () => {this.sound.stopByKey("music_menu"), this.click.play(), this.scene.start("level2")} );

        var button_restart = this.add.image(315, 570, "button_restart")
            button_restart.setInteractive()
            button_restart.on("pointerdown", () => {this.sound.stopByKey("music_menu"), this.click.play(), this.scene.start("level1")} );
